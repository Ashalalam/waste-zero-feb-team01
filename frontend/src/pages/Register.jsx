...
const S = {
  card: {
    ...
    boxShadow: "0 0 0 1px rgba(255,255,255,.07), 0 8px 32px rgba(0,0,0,.4), 0 32px 80px rgba(0,0,0,.5)",
  },
  left: {
    flex: "0 0 35%",
  ...
  ring: (w, h, top, left, bottom, right, op) => ({
    ...
    top, left, bottom, right, pointerEvents: "none",
  }),
  logoWrap: {
  ...
  right: {
    ...
    padding: "36px 48px",
    background: "linear-gradient(160deg, #fdf8f0 0%, #f9f3e8 60%, #f4ede0 100%)",
  ...
  tabs: {
    ...
    background: T.bPale, padding: "4px", marginBottom: "20px", gap: "4px",
  },
  tab: (a) => ({
    flex: 1, padding: "9px", borderRadius: "8px", border: "none",
    fontFamily: font, fontSize: "14px",
    fontWeight: a ? 600 : 500, cursor: "pointer",
    background: a ? T.cream : "transparent",
    color: a ? T.bDark : T.textSoft,
    boxShadow: a ? "0 1px 4px rgba(62,39,35,.1)" : "none",
    transition: "all .2s",
  }),
  h2: { fontFamily: serif, fontSize: "24px", fontWeight: 800, color: T.bDark, marginBottom: "4px", letterSpacing: "-.3px", position: "relative" },
  sub: { fontSize: "13.5px", color: T.textSoft, marginBottom: "20px", position: "relative" },
  grid2: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "14px" },
  field: { marginBottom: "14px" },
  label: { display: "block", fontSize: "12px", fontWeight: 700, color: T.bMid, marginBottom: "5px", letterSpacing: ".4px", textTransform: "uppercase" },
  input: {
    width: "100%", padding: "11px 14px",
    border: `1.5px solid ${T.bSand}`, borderRadius: "11px",
    fontSize: "14px", fontFamily: font,
    color: T.textDark, background: "rgba(255,255,255,.7)",
  ...
  btn: (loading) => ({
    ...
    background: loading ? T.bSand : `linear-gradient(135deg, ${T.gMid} 0%, ${T.gDark} 60%, ${T.bMid} 100%)`,
    color: loading ? "#a09080" : "#fff",
    border: "none", borderRadius: "11px",
    fontSize: "15px", fontWeight: 600, fontFamily: font,
    cursor: loading ? "not-allowed" : "pointer",
    marginTop: "6px", marginBottom: "16px",
    boxShadow: loading ? "none" : "0 4px 16px rgba(46,125,50,.35), inset 0 1px 0 rgba(255,255,255,.1)",
    transition: "opacity .2s, transform .15s",
  }),
  error: { background: "#fef2f2", border: "1px solid #fecaca", color: "#c62828", borderRadius: "10px", padding: "11px 14px", fontSize: "13.5px", marginBottom: "16px" },
  link: { color: T.gDark, textDecoration: "none", fontWeight: 700 },
  pwWrap: { position: "relative" },
  eyeBtn: { position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", fontSize: "16px", cursor: "pointer", color: T.bLight, lineHeight: 1 },
};

const focusOn  = (e) => { e.target.style.borderColor = T.gMid; e.target.style.boxShadow = "0 0 0 3px rgba(67,160,71,.15)"; e.target.style.background = "rgba(255,255,255,.95)"; };
const focusOff = (e) => { e.target.style.borderColor = T.bSand; e.target.style.boxShadow = "none"; e.target.style.background = "rgba(255,255,255,.7)"; };
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try { await registerUser(formData); navigate("/"); }
  ...
  const inp = (name, placeholder, type = "text", extra = {}) => (
    <input type={type} name={name} placeholder={placeholder} value={formData[name]}
      onChange={handleChange} onFocus={focusOn} onBlur={focusOff}
      style={{ ...S.input, ...extra }} required />
  );
